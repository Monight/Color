...
public class CreationVineEntity : Editor
{
    ...
    public void OnEnable()
    {
        ...
        //Debug.Log(ver);
        //Debug.Log(vine.MoveVertexs +"  "  + vine);
        //Debug.Log(vine.MoveVertexs.Count);
        if (vine.MoveVertexs == null || vine.MoveVertexs.Count == 0)
        {
            MoveVertexs = new List<Vector3>();
            MoveVertexs.Add(new Vector3(-1 + ver.x, ver.y, 0)); //如果没有的话  取两个点
    ...
    public override void OnInspectorGUI()
    {
        ...
        Debug.Log( "acceptInput == "+  acceptInput);
        if (acceptInput)
        {
        ...
        else
        {
            ...
            if (GUILayout.Button("开 始 Vine 编 辑"))
            {
                //line.enabled = true;
                acceptInput = true;
                SceneView.RepaintAll(); //绘制所有的
            }
        }
    }
    Vector3 tp;
    public void OnSceneGUI() // 一个回调 其实是
    {
        Debug.Log("从哪里来的啊 ");
        if (!acceptInput)
            return;
